...
"""
##############################################################################
# Main script of the SC-2-FC mapping
##############################################################################
...
single_layer = True
if single_layer:
    selected_layer_setup = all_single_layer_setup
else:
    selected_layer_setup = all_layer_setup

# for single layer setup, change the code below as needed
for layer_setup in selected_layer_setup:
    if single_layer:
    ...
    for hyperparamters in all_hyperparamters:
        ...
        EPOCHS = 3000
        lamb = 0.1 # tuning paramter controlling trade-off between reconstruction and classification
        ...
        for it in range(folds):
            ...
            with tf.Session() as sess:
                ...
                saver.save(sess, './models/' + directory_name + '_gcn_encoder_decoder_system' + '_fold_' + str(it))
                print("Model saved") 
        ...
        all_results = [MSE_results, CLA_results, TPR_results, TNR_results, Precision_results, \
                    ...
                    TPs, TNs, FPs, FNs]
        with open('results/' + directory_name + '.pkl', 'wb') as f:
